function rss_CSimage = processKspaceCS(data)
   ...
   factor = 2;
    ...
    for frame = 1:num_frames
        for coil = 1:num_coils
            ...
            for iter = 1:num_iters
                [C, S] = wavedec2(real(im_cs(:,:,coil,frame)), 2, 'db1');
                %C = single(C);

                C_thresh = wthresh(C, 's', threshold);
                im_cs(:,:,coil,frame) = complex(waverec2(C_thresh, S, 'db1'), 0);
    ...
    save(fullfile(pwd,'Cuda_reconstructed_CS_image.mat'), 'rss_CSimage');
